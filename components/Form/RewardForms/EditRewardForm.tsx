...
const EditRewardForm = ({ reward }: RewardProps) => {
  const ExplicitRewardSchema = RewardSchema.refine(
    (data) =>
      data.isExistingImage === false &&
      data.isCustomDateRange ===
        (reward.startDate !== null && reward.endDate !== null)
  );
  const form = useForm<z.infer<typeof RewardSchema>>({
  ...
  const [isDialogOpen, setDialogOpen] = useState(false);
  const onSubmit = (values: z.infer<typeof ExplicitRewardSchema>) => {
    startTransition(async () => {
      const formData = new FormData();
      formData.append("name", values.name);
      formData.append("pointsRequired", values.pointsRequired.toString());
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormLabel>Use existing image</FormLabel>
                  </div>
                </FormItem>
              )}
            />
          ...
          <FormItem>
            ...
            <RadioGroup
              ...
              defaultValue={isCustomDateRange.toString()}
            >
              <div className="flex items-center space-x-2">
